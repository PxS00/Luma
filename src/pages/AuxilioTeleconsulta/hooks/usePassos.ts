...
export function usePassos(total: number, start = 0) {
  ...
  function prev() { if (canPrev) setIdx(i => i - 1); }//* Voltar
  function next() { if (canNext) setIdx(i => i + 1); } //* Próximo
  function go(i: number) { if (i >= 0 && i < total) setIdx(i); } //* Ir para passo específico
